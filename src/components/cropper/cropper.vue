...
export default {
  ...
  methods: {
    ...
    draw() {
      ...
      this.$emit('getImage', this.$refs.canvas.toDataURL());
    },
    // 4
    generateDrwaImageParam() {
      var source = {
        x: 0,
        y: 0,
        width: 0,
    ...
    moveRightBottom(offsetX, offsetY) {
    ...
    moveLeftBottom(offsetX, offsetY) {
      var minHeight = baseWidth;
      var curL = this.cropBox.left + offsetX;
      var curH = this.cropBox.height + offsetY;
      // 边界检查
