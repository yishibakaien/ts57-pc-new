...
<div class="ts-carousel" @mouseover="listEvent" @mouseout="listEvent">
  <!-- 箭头 - 左 -->
  <button class="ts-carousel-arrow left" :class="['is-'+arrow,'is-'+arrowType]" @click="arrowEvent(-1)">
  ...
  <!-- 箭头 - 右 -->
  <button class="ts-carousel-arrow right" :class="['is-'+arrow,'is-'+arrowType]" @click="arrowEvent(1)">
...
export default {
  ...
  watch: {
    ...
    value(val, oldVal) {
      if (this.fade) {
        this.$children.forEach((item, i, children) => {
          if (val !== oldVal) {
            item.$el.style.opacity = 0;
          }
          children[val].$el.style.opacity = 1;
        });
      }
    ...
    if (this.fade) {
      this.$children.forEach((item, i, children) => {
        if (i !== this.currentIndex) {
          children[i].$el.style.opacity = 0;
        }
        children[this.currentIndex].$el.style.opacity = 1;
      });
    }
  ...
  computed: {
    trackStyles() {
      return {
        ...
        transition: this.fade ? '' : `transform .8s ${this.easing}`
      };
    }
  }
};
