    ...
    <div class="ts-form-box">
      <ts-form :model="addBuyForm" :rules="rules" ref="addBuyForm" label-width="140px" label-position="left">
        ...
        <ts-form-item label="求购数量：" prop="buyNum">
          <ts-input :maxlength='9' v-model="addBuyForm.buyNum" width="320px" ></ts-input>
...
export default {
  ...
  methods: {
    ...
    uploadImg(e) {
      // 放到表单
      this.addBuyForm.buyPicUrl = ALI_DOMAIN + e[0].storeAs;
    },
    // 提交表单
    submitForm(formName) {
      this.$refs[formName].validate(async(valid) => {
        if (valid) {
          let res = await releaseProductBuy(this.addBuyForm);
          if (!res.data.code) {
            this.modelShow = true;
          }
        } else {
