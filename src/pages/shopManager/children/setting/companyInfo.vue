...
export default {
  ...
  methods: {
    async editCompany(formName) {
      if (!this.Text.show) {
        this.$refs[formName].validate(async(valid) => {
          if (valid) {
            await updateCompanyExtend({
              companyBusiness: this.companyInfoForm.companyExtendBO.companyBusiness
            });
    ...
    // 上传图片
    uploadAvatar(e) {
      this.companyInfoForm.companyHeadIcon = ALI_DOMAIN + e[0].storeAs;
    },
    // 上传店招
    uploadBanner(e) {
      this.companyInfoForm.companyBanner = ALI_DOMAIN + e[0].storeAs;
    },
    // 上传厂家风采
    uploadViews(e) {
      this.companyInfoForm.presence.push({
        picUrl: ALI_DOMAIN + e[0].storeAs,
        picName: e[0].name
      });
    },
    // 删除厂家风采图片
