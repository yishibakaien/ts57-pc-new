...
<div class="warehouse" v-loading.fullscreen.lock="fullscreenLoading" element-loading-text="请稍候">
  <ts-section>
    ...
    <div slot="menu">
      ...
      <label>
        ...
        <ts-aliupload fileType="3" id="uploadProduct" @begin="fullscreenLoading = true" multiple @doUpload="uploadProduct" ></ts-aliupload>
      </label>
    </div>
    <!-- ======== -->
    <!-- 品种过滤器 -->
          ...
          <ts-col :span="12" class="warehouse-grid" v-for="item in productList.list" :key="item.id">
            <div class="warehouse-grid-item"  v-loading.body="item.loading">
              <ts-row>
                <ts-col :span="7">
                  ...
                  <div class="warehouse-grid-item--status">
                    <img class="warehouse-grid-item--img" :src="`/static/images/status/${item.publishStatus}.png`" alt="">
                ...
                <ts-col :span="2">
                  ...
                  <ul class="warehouse-grid-item--navbar">
                    <li @click="handleTabChange(item,1)"  :class="[item.isActive===1?'is-active':'']">基本信息</li>
                ...
                <ts-col :span="15">
                  ...
                  <div class="warehouse-grid-item--container" v-if="item.isActive===1">
                    ...
                    <ts-row class="warehouse-grid-item--row">
                      ...
                      <ts-col :span="7"><span>{{item.productShape | filterDict(dicTree.PRODUCT_SHAPE,'name')}}&nbsp;</span></ts-col>
                  ...
                  <div class="warehouse-grid-item--container"  v-else>
                    ...
                    <div class="warehouse-product-color">
                      <div v-if="item.colorCardsNum<5">
                        <span v-for="color in item.colorCardspic">
                            <ts-image :src="cards.picUrl" v-for="cards in color" height="60" width="60" :canView="false" disabledHover class="warehouse-product-color--image" @click="showColorDetail(cards,item)"></ts-image>
                        </span>
                      </div>
                      <div v-if="item.colorCardsNum>=5">
                        <ts-carousel class="warehouse-product-color--carousel" v-model="item.carousel" dots="never" arrow="always" arrowType="square" easing='linear' >
                          <ts-carousel-item v-for="color in item.colorCardspic">
                            <div class="item">
                              <ts-image :src="cards.picUrl" v-for="cards in color" height="60" width="60" :canView="false" disabledHover @click="showColorDetail(cards,item)"></ts-image>
                            </div>
                          </ts-carousel-item>
                        </ts-carousel>
                      </div>
                    ...
                    <ts-row class="warehouse-product-color--row">
                      ...
                      <ts-col :span="7">{{item.colorDetail.isStock |filterDict(DICT.isStock)}}</ts-col>
              ...
              <div class="warehouse-grid-item--footer">
                <ts-button type="primary" v-if="item.publishStatus!==1" @click="handleShelveProduct({goal:1,ids:item.id,isUp:true})" size="small">上架网店</ts-button>
                <ts-button type="primary" v-if="item.publishStatus!==2&&getIsStore" @click="handleShelveProduct({goal:2,ids:item.id,isUp:true})" size="small">上架平台</ts-button>
                <ts-button type="primary" v-if="item.publishStatus!==0" @click="handleShelveProduct({goal:0,ids:item.id})" size="small">下架仓库</ts-button>
    ...
    <div slot="footer" class="warehouse-footer">
      <div>
        ...
        <span v-if="Filter.publishStatuss!==null">
          <ts-button type="primary" :disabled="getChooseIemLen" v-if="Filter.publishStatuss!==2&&getIsStore" @click="handleShelveProduct({goal:2,ids:chooseItem,isUp:true})">上架平台</ts-button>
          <ts-button type="primary" :disabled="getChooseIemLen" v-if="Filter.publishStatuss!==1" @click="handleShelveProduct({goal:1,ids:chooseItem,isUp:true})">上架网店</ts-button>
          <ts-button type="cancel" :disabled="getChooseIemLen" v-if="Filter.publishStatuss!==0" @click="handleShelveProduct({goal:0,ids:chooseItem})">下架仓库</ts-button>
        </span>
        <ts-button type="cancel" :disabled="getChooseIemLen" @click="handleShowDialog(chooseItem)">删除</ts-button>
      </div>
      <!-- === -->
      <!-- 分页 -->
      <!-- ===-->
  ...
  <!--  对话框 - 批量上传 -->
  <!-- ================ -->
  <ts-dialog :close-on-click-modal="!loading" type="alert" width="80%" v-model="BatchUpload.dialogShow" title="请选择上传花型的属性（*为必填项）" @close="loadProductList">
