  ...
  <ts-form :model="companyInfoForm" :rules="rules" ref="companyInfoForm" label-width="125px" label-position="left" class="companyInfo-container">
    <div class="companyInfo-container-col">
      <ts-form-item label="注册资本：">
        <p v-if="Text.show" class="companyInfo-formItem-text">{{companyInfoForm.registeredMoney}}</p>
        <!-- <p v-if="Text.show" class="companyInfo-formItem-text">{{companyInfoForm.registeredMoney | filterDict(DICT.RegisteredMoney)}}</p> -->
        <ts-select v-else style="width:250px" data-key-name="label" data-val-name="label" placeholder="选择单位" :options='DICT.RegisteredMoney' v-model="companyInfoForm.registeredMoney"></ts-select>
      </ts-form-item>
      <ts-form-item label="企业类型：">
        ...
        <p v-if="Text.show" class="companyInfo-formItem-text">{{companyInfoForm.canpanyType}}</p>
        <ts-select style="width:250px" data-key-name="label" data-val-name="label" placeholder="选择单位" :options='DICT.CompanyType' v-model="companyInfoForm.canpanyType" v-else></ts-select>
      </ts-form-item>
      <ts-form-item label="员工数量：">
        <p v-if="Text.show" class="companyInfo-formItem-text">{{companyInfoForm.nop }}</p>
        <!-- <p v-if="Text.show" class="companyInfo-formItem-text">{{companyInfoForm.nop | filterDict(DICT.Nop)}}</p> -->
        <ts-select v-else style="width:250px" data-key-name="label" data-val-name="label" placeholder="选择单位" :options='DICT.Nop' v-model="companyInfoForm.nop"></ts-select>
      </ts-form-item>
      <ts-form-item label="年营业额：">
        <p v-if="Text.show" class="companyInfo-formItem-text">{{companyInfoForm.turnover}}</p>
        <!-- <p v-if="Text.show" class="companyInfo-formItem-text">{{companyInfoForm.turnover | filterDict(DICT.Turnover)}}</p> -->
        <ts-select style="width:250px" data-key-name="label" data-val-name="label" placeholder="选择单位" :options='DICT.Turnover' v-model="companyInfoForm.turnover" v-else></ts-select>
      </ts-form-item>
    ...
    <div class="companyInfo-container-col">
      <ts-form-item label="成立时间：">
        ...
        <ts-date-picker v-if="!Text.show" placeholder="选择日期" v-model="companyInfoForm.createDate" style="width:250px" backward></ts-date-picker>
      </ts-form-item>
      ...
      <ts-form-item label="设备数量：" prop="machineNum">
        <p v-if="Text.show" class="companyInfo-formItem-text">{{companyInfoForm.machineNum}}</p>
        <!-- <p v-if="Text.show" class="companyInfo-formItem-text">{{companyInfoForm.machineNum | filterDict(DICT.MachineNum)}}</p> -->
        <ts-select style="width:250px" data-key-name="label" data-val-name="label" placeholder="选择单位" :options='DICT.MachineNum' v-model="companyInfoForm.machineNum" v-else></ts-select>
      </ts-form-item>
    </div>
  </ts-form>
</ts-section>
