...
<ts-section :pageTitle="title">
  <ts-form :model="addSupplyForm" :rules="rules" ref="addSupplyForm" label-width="125px" label-position="left">
    <ts-form-item label="花型图片：" v-loading.body="loading" element-loading-text="正在上传中" prop="productPicUrl">
      ...
      <label class="add-upload-button">
          {{Pic.text}}
          <aliUpload fileType="3" id="addPic" @begin="handleUpload" @doUpload="uploadImg"></aliUpload>
        </label>
    </ts-form-item>
...
export default {
  ...
  methods: {
    ...
    uploadImg(e) {
      ...
      // 显示Base64
      // 放到表单
      this.addSupplyForm.productPicUrl = ALI_DOMAIN + e[0].storeAs;
    },
    // 提交表单
    submitForm(formName) {
