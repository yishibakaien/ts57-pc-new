    ...
    <ts-form :model="addPatternForm" :rules="rules"  ref="addPatternForm" label-width="125px" label-position="left" >
      <ts-row>
        <ts-col :span="6" v-if="!isCreatedStatus" class="center">
          <ts-image width="200" height="200" :src="Color.detail.picUrl" v-show="Pic.show" :canView="false" disabledHover>
          </ts-image>
          <p>
            <br>
        ...
        <ts-col :span="isCreatedStatus?24:18" class="add-col" :style="{'max-height':`${getClientHeight/1.5}px`}">
        <ts-menu-table>
            ...
            <ts-col :span="12">
              <ts-form-item label="大货类型：" :prop="validate?'productShape':''">
                <ts-radio-group bordered v-model="addPatternForm.productShape" >
                  <ts-radio  :label="item.dicValue" :key="item.value" v-for="item in dicTree.PRODUCT_SHAPE">{{item.name}}</ts-radio>
                </ts-radio-group>
              </ts-form-item>
            </ts-col>
          </ts-row>
          <ts-form-item label="花型成分：" :prop="validate?'ingredient':''">
            <ts-radio-group bordered v-model="addPatternForm.ingredient">
              ...
              <!-- 新增的成分 -->
                <ts-input :validateEvent="false" placeholder="自定义成分" @keyup.enter.native="handleAddIngredient" class="add-input" v-model="EditIngredient.newIngredient" :maxlength="8" size="small" v-clickoutside="handleAddIngredient"></ts-input>
            </ts-radio-group>
          </ts-form-item>
        ...
        <div v-if="!isCreatedStatus">
          ...
          <ts-row :gutter="10">
            <ts-col :span="12">
              <ts-form-item label="剪版价：" prop="cutPrice">
                <ts-input v-model="cutPrice" style="width:50%" :maxlength='9' placeholder="请输入单价"></ts-input>
                <ts-select style="width:48%" data-key-name="name"  data-val-name="dicValue" placeholder="选择单位" :options='CopyDICTUnit' v-model="addPatternForm.priceUnit"></ts-select>
              </ts-form-item>
            </ts-col>
            <ts-col :span="12">
              <ts-form-item label="大货价：" prop="price">
                <ts-input v-model="prince" style="width:50%" :maxlength='9' placeholder="请输入单价"></ts-input>
                <ts-select style="width:48%" data-key-name="name"  data-val-name="dicValue" placeholder="选择单位" :options='CopyDICTUnit' v-model="addPatternForm.priceUnit"></ts-select>
              </ts-form-item>
            </ts-col>
          </ts-row>
          <ts-row>
            <ts-col :span="12">
              <ts-form-item label="出码率：" prop="outRate">
                <ts-input v-model="addPatternForm.outRate" :maxlength='10' placeholder="请输入出码率"></ts-input>
              </ts-form-item>
            </ts-col>
          </ts-row>
        </div>
          ...
          <!-- 添加色卡 -->
          <add-color v-if="!isCreatedStatus" :ColorList="Color.list" :product="addPatternForm" :unit="CopyDICTUnit" @setColorList="getColorList" @saveColor="handleSaveColor"></add-color>
        </ts-col>
      </ts-row>
    </ts-form>
    <div class="add-bottom-button">
      <ts-button type="primary"  @click="submitForm('addPatternForm')">{{isCreatedStatus?'批量上传图片':'保存修改'}}</ts-button>
      <ts-button slot="footer" type="plain" @click="handleUplace('addPatternForm')" v-if="upLace">{{!addPatternForm.isShelve?'上架蕾丝控':'下架蕾丝控'}}</ts-button>
    </div>
  </div>
</template>
