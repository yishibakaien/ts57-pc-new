...
<div class="threeD">
  <threed-dress :chooseItem="Choose.confirm">
    <ts-button slot="uploadBtn" type="primary" @click="Dialog.show = true">
       <i class="icon-shangchuan"></i> 上传店铺花型
    </ts-button>
    <div slot="productMenu" class="models-productMenu onepx-t">
      <ts-button v-show="!!userInfo.id" :disabled="Collect.disabled" @click="handleCollectProduct">
  ...
  <ts-dialog v-model="Dialog.show" width="95%" title="上传店铺花型" @confirm="handleChooseProduct" @cancel="handleCancelChoose">
  ...
  <ts-tab-container v-model="selected" class="models-tab-container">
    ...
    <ts-tab-container-item id="2">
      <ts-grid :data="Product.supplies.list">
        <ts-grid-item style="width:200px" v-for="(product,index) in Product.supplies.list" :key="product" @click="handleChooseStore(product,index)" :class="{'models-active':index===Choose.index}">
          <ts-image
           width="170"
           height="170"
           :canView="false"
...
export default {
  ...
  created() {
    ...
    let params = {
      pageNO: 1,
      pageSize: 10,
      companyId: this.Params.companyId = this.isShop && !this.productDetail.companyId ? this.$route.params.id : this.productDetail.companyId
    };
